...
public class ProjectViewModel : ObservableRecipient, INavigationAware
{
    ...
    public void OnNavigatedTo(object parameter)
    {
        if (parameter is ProjectSolutionModel data)
        {
            ...
            /*
               ObservableCollection<ExplorerItemViewModel> result = new();
            Task.Run(async () =>
            {
                var data = await getProjectData.GetProjectSolutionItens(solutionPath);
                result = new ObservableCollection<ExplorerItemViewModel>(data.Select(i => new ExplorerItemViewModel(i)));
            }).Wait();
