...
public sealed record MoveFileItemCommand(
    string OldPath
    , string NewPath
...
public sealed class MoveFileItemCommandHandler : IHandler<MoveFileItemCommand, Result<Empty>>
{
    ...
    public MoveFileItemCommandHandler(
    ...
    public async Task<Result<Empty>> HandleAsync(
        MoveFileItemCommand query
        , CancellationToken cancellation
    ) => await Task.Run(() => _repository.MoveFileItem(query.OldPath, query.NewPath));
}