...
public class RandomizedWorldPointsBehavior : Behavior<Canvas>
{
                ...
                while (!cancellationToken.IsCancellationRequested)
                {
                    try
                    {
                        var locations = WorldLocations
                            .OrderBy(keySelector: _ => RandomSource.NextDouble())
                            .Take(count: _targetControls.Count);
                        ...
                        Task.WaitAll(
                            tasks: cities.Select(selector: x => AnimateCityMarkerAsync(target: x.control, point: x.point
    ...
    )
    {
        ...
        await Task.Delay(delay: TimeSpan.FromSeconds(value: 1), cancellationToken: cancellationToken);
        ...
        await Task.Delay(delay: TimeSpan.FromSeconds(value: 2), cancellationToken: cancellationToken);
        ...
        Dispatcher.UIThread.Post(
            action: () =>
            {
                ...
                var targets = AssociatedObject.Children
                    .Where(predicate: x => x.Classes.Contains(item: "City"))
                    .ToList();
                ...
                RunAnimation(cancellationToken: _cts.Token);
            },
