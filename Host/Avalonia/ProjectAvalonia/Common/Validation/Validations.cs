...
public class Validations : ReactiveObject, IRegisterValidationMethod, IValidations
{
    ...
    IEnumerable<string> IValidations.Infos => ErrorsByPropertyName.Values
        .SelectMany(x => x.Where(error => error.Severity == ErrorSeverity.Info)
    .Select(error => error.Message));

    IEnumerable<string> IValidations.Warnings => ErrorsByPropertyName.Values
        .SelectMany(x => x.Where(error => error.Severity == ErrorSeverity.Warning)
        .Select(error => error.Message));

    IEnumerable<string> IValidations.Errors => ErrorsByPropertyName.Values
        .SelectMany(x => x.Where(error => error.Severity == ErrorSeverity.Error)
        .Select(error => error.Message));
