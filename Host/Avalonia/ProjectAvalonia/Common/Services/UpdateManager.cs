...
public class UpdateManager : IDisposable
{
    ...
    private async void UpdateChecker_UpdateStatusChangedAsync(object? sender, UpdateStatus updateStatus)
    {
        ...
        if (DownloadNewVersion && !RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
        {
            do
            {
                ...
                try
                {
                    (var installerPath, var newVersion) = await GetInstallerAsync(targetVersion).ConfigureAwait(false);
    ...
    private async Task<(Version LatestVersion, string InstallerDownloadUrl, string InstallerFileName, string Sha256SumsUrl, string WasabiSigUrl)> GetLatestReleaseFromGithubAsync(Version targetVersion)
    {
        ...
        var softwareVersion = jsonResponse["tag_name"]?.ToString() ?? throw new InvalidDataException("Endpoint gave back wrong json data or it's changed.");
        ...
        var assetsInfos = jsonResponse["assets"]?.Children().ToList() ?? throw new InvalidDataException("Missing assets from response.");
    ...
    private void EnsureToRemoveCorruptedFiles()
    {
        ...
        if (folder.Exists)
        {
            var corruptedFiles = folder.GetFileSystemInfos().Where(file => file.Extension.Equals(".tmp"));
